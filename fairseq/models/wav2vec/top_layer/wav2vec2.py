...
@dataclass
class Wav2Vec2TopLConfig(FairseqDataclass):
    ...
    update_top_l: int = field(
        default=12, metadata={"help": "只更新transformer encoder中的top l层的参数, 为了加快运算"}
    )
...
@register_model("wav2vec2_top_l", dataclass=Wav2Vec2TopLConfig)
class Wav2Vec2Model(BaseFairseqModel):
    def __init__(self, cfg: Wav2Vec2TopLConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.encoder = TransformerEncoderTopL(cfg)
    ...
    @classmethod
    def build_model(cls, cfg: Wav2Vec2TopLConfig, task=None):
...
class TransformerEncoderTopL(nn.Module):
    ...
    def extract_features(self, x, padding_mask=None):
        ...
        for i, layer in enumerate(self.layers):
            if i < len(self.layers) - self.top_l: # 0, 1, 2, ..., 5
                with torch.no_grad():
