...
@register_model("wav2vec_ctc_ad_stack", dataclass=Wav2Vec2CtcADConfig)
...
class Wav2VecEncoderAD(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrADConfig, tgt_dict=None):
        ...
        w2v_args.model['_name'] = 'wav2vec2_ad_stack'
        ...
        if tgt_dict is not None:
            self.proj = Linear(d, 185)

            self.proj_lang_specific = nn.Linear(185 , len(tgt_dict))  # szj added
            nn.init.constant_(self.proj_lang_specific.bias, 0.0)
            # 权重初始化
            my_weight = nn.Parameter(torch.zeros(98, 185), requires_grad=True)
            with torch.no_grad():
                for i in range(98):  # 对应空格符, sos, eos, pad, unk, 越南语 4, 5,  ..., 97
                    my_weight[i, i] = 1
                self.proj_lang_specific.weight.copy_(my_weight)
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        if self.proj:
            x = self.proj(x)
            x = self.proj_lang_specific(x)  # szj
