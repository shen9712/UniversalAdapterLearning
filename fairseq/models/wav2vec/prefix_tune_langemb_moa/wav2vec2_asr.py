...
@register_model("wav2vec_ctc_prefix_tune_lang_emb_moa", dataclass=Wav2Vec2CtcADConfig)
...
class Wav2VecEncoderAD(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrADConfig, tgt_dict=None):
        ...
        w2v_args.model['_name'] = 'wav2vec2_prefix_tune_lang_emb_moa'
        ...
        for name, param in self.w2v_model.named_parameters():
            if param.requires_grad and 'adapter_list' in name:
                if 'adapter_list.3' not in name:  # 2对应库尔德语, 3皮金语
                    param.requires_grad = False
