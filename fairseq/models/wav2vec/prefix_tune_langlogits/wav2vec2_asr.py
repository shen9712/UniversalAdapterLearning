...
class Wav2Vec2AsrADConfig(FairseqDataclass):
    ...
    freeze_lid: bool = field(
        default=True, metadata={"help": "no更新语种识别器"}
    )
    # szj
...
@register_model("wav2vec_ctc_prefix_tune_lang_logits", dataclass=Wav2Vec2CtcADConfig)
...
class Wav2VecEncoderAD(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrADConfig, tgt_dict=None):
        ...
        w2v_args.model['_name'] = 'wav2vec2_prefix_tune_lang_logits'
        ...
        with open_dict(w2v_args):  # 对于omegaconf, 新增参数的写法, refer: https://stackoverflow.com/questions/66295334/create-a-new-key-in-hydra-dictconfig-from-python-file
            ...
            w2v_args.model.freeze_lid = cfg.freeze_lid
        ...
        if self.freeze_lid:
            for name, param in self.w2v_model.named_parameters():
                if param.requires_grad and 'lang_' in name:
                    param.requires_grad = False
                if 'lang_' not in name:
                    param.requires_grad = True
        else:
            for name, param in self.w2v_model.named_parameters():
                if param.requires_grad and 'lang_' not in name:
                    param.requires_grad = False
                if 'lang_' in name:
                    param.requires_grad = True
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        with torch.no_grad() if not ft else contextlib.ExitStack():
            x, padding_mask, language_logits = self.w2v_model.extract_features(**w2v_args)
