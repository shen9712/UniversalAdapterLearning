...
# szj
from fairseq.models.wav2vec.prefix_tune.attention import multi_head_attention_forward
...
class PrefixTuning(nn.Module):
    def __init__(
        ...
        self.input_tokens = torch.arange(self.prefix_length).long()  # 词汇量为30，对应30个embedding
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        # self.prefix_tune = PrefixTuning(n_layers=1, n_heads=num_heads, input_size=self.embed_dim)
        self.prefix_tune = FlatPrefixTuning(n_layers=1, n_heads=num_heads, input_size=self.embed_dim)
    ...
    def forward(
        ...
        if (
            ...
            # szj 对key value进行处理
            prefix_keys_values = self.prefix_tune(query.shape[1])  # (2 x bsz x n_heads x prefix_length x n_embd_per_head)
            prefix_keys = prefix_keys_values[0][0, ...]  # (bsz, n_heads, prefix_length, n_embd_per_head)
            prefix_values= prefix_keys_values[0][1, ...]  # (bsz, n_heads, prefix_length, n_embd_per_head)

            bsz = query.shape[1]
            # prefix_keys = prefix_keys.permute(2, 0, 1, 3).contiguous().reshape(self.prefix_tune.prefix_length, bsz, -1)
            # prefix_values = prefix_values.permute(2, 0, 1, 3).contiguous().reshape(self.prefix_tune.prefix_length, bsz, -1)
            # key = torch.cat([prefix_keys, key], dim=0)
            # value = torch.cat([prefix_values, value], dim=0)

            # szj 对key padding mask进行处理
            # prefix_mask = torch.zeros((bsz, prefix_keys.size(0)), dtype=torch.bool).to(key_padding_mask.device)  # (b x t)
            # key_padding_mask = torch.cat([prefix_mask, key_padding_mask], dim=1)
