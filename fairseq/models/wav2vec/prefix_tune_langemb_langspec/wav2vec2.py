...
# szj
from fairseq.models.wav2vec.prefix_tune_langemb_langspec.multihead_attention import MultiheadAttention
...
@register_model("wav2vec2_prefix_tune_lang_emb_langspec", dataclass=Wav2Vec2ADConfig)
    ...
    def extract_features(self, x, padding_mask=None, num_updates=None, language_id=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                x, z = layer(x, self_attn_padding_mask=padding_mask, need_weights=False, num_updates=num_updates, prefix_keys_values=language_prefix[i])
...
class PrefixTuningLangEmbLangspec(nn.Module):
    ...
    def forward(self, batch_size, language_id):
        ...
        # szj 每个语种一套参数
        embs = self.control_trans(embs)  # b x 1 x input_size
        output = []
