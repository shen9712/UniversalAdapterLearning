...
@dataclass
class Wav2Vec2ADLANGINVConfig(FairseqDataclass):
...
class NICECouplingBlock(nn.Module):
    ...
    def __init__(self, dims_in, dims_c=[], non_linearity="relu", reduction_factor=2):
        ...
        subnet_constructor = self.get_subnet_constructor(non_linearity, reduction_factor)
    ...
    def get_subnet_constructor(self, non_linearity, reduction_factor):
...
@register_model("wav2vec2_ad_lang_inv", dataclass=Wav2Vec2ADLANGINVConfig)
class Wav2Vec2ADModel(BaseFairseqModel):
    def __init__(self, cfg: Wav2Vec2ADLANGINVConfig):
        super().__init__()
        self.cfg = cfg
        ...
        # szj added invertible adapter
        self.inv_adapter = NICECouplingBlock(dims_in=[[cfg.encoder_embed_dim]])
    ...
    @classmethod
    def build_model(cls, cfg: Wav2Vec2ADLANGINVConfig, task=None):
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False):
        ...
        # szj added invertible adapter
        x = self.inv_adapter(x, rev=False) # B x T x C

        x = self.encoder(x, padding_mask=padding_mask)

        if features_only:
            # szj added
            x = self.inv_adapter(x, rev=True)  # B x T x C
