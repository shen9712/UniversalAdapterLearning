...
@register_model("wav2vec_ctc_ad_parallel_gate", dataclass=Wav2Vec2CtcADConfig)
class Wav2VecCtcAD(BaseFairseqModel):
    ...
    def get_tfm_gate_regu_loss(
        self,
        net_output,
    ):
        """
        tfm_running_gate_layer_results: n_layers x [ T x B x D ]
        """
        tfm_running_gate_layer_results = net_output['tfm_running_gate_layer_results']
        
        weight = 1
        loss = 0
        
        tfm_running_gate = torch.stack(tfm_running_gate_layer_results)  # n_layers x T x B x D
        padding_mask = tfm_running_gate != 0
        tfm_running_gate = torch.masked_select(tfm_running_gate, padding_mask)  #  把非mask的位置取出来
        loss = tfm_running_gate.mean()
...
class Wav2VecEncoderAD(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrADConfig, tgt_dict=None):
        ...
        w2v_args.model['_name'] = 'wav2vec2_ad_parallel_gate'
        ...
        for name, param in self.w2v_model.named_parameters():
            if param.requires_grad and 'adapter' not in name and 'gate' not in name:
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        with torch.no_grad() if not ft else contextlib.ExitStack():
            x, padding_mask, tfm_running_gate_layer_results = self.w2v_model.extract_features(**w2v_args)
        ...
        return {
            ...
            "padding_mask": padding_mask,
            "tfm_running_gate_layer_results": tfm_running_gate_layer_results,
        }
