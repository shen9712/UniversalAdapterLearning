...
# szj
from fairseq.models.wav2vec.prefix_tune_langemb_adapter_madg_share.attention import multi_head_attention_forward
...
class PrefixTuningLangEmb(nn.Module):
    ...
    def forward(self, batch_size, input_tokens):
        # device = next(self.parameters()).device
        # input_tokens = self.input_tokens.unsqueeze(0).expand(batch_size, -1).to(device)
        key_values = input_tokens.unsqueeze(-1).unsqueeze(-1) # .to(device)  # (b x 1 x 1)
        del input_tokens  # 节省显存？
        key_values = self.wte(key_values)  # (b x 1 x input_size)
        key_values = self.control_trans(key_values)  # batch_size x prefix_length x n_layers*2*input_size
        key_values = key_values.view(
        ...
        key_values = key_values.permute(2, 0, 3, 1, 4).contiguous().split(2)
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.prefix_tune = PrefixTuningLangEmb(n_layers=1, n_heads=num_heads, input_size=self.embed_dim)
    ...
    def forward(
        ...
        if (
            ...
            # szj 对key value进行处理
            prefix_keys_values = self.prefix_tune(query.shape[1], language_id)  # (2 x bsz x n_heads x prefix_length x n_embd_per_head)
