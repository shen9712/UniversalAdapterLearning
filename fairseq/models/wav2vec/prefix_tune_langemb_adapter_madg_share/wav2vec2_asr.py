...
@register_model("wav2vec_ctc_prefix_tune_lang_emb_adapter_madg_share", dataclass=Wav2Vec2CtcADConfig)
class Wav2VecCtcAD(BaseFairseqModel):
    ...
    # szj 增加cpg正则项
    def get_cpg_regu_loss(
        self,
        net_output,
    ):
        cpg_regu_weight = net_output['cpg_regu_weight']
        target_mat = torch.eye(cpg_regu_weight.shape[0], device=cpg_regu_weight.device, dtype=cpg_regu_weight.dtype)

        loss_weight = 1
        regu_loss = (cpg_regu_weight - target_mat).pow(2).sum()

        return regu_loss * loss_weight
...
class Wav2VecEncoderAD(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrADConfig, tgt_dict=None):
        ...
        w2v_args.model['_name'] = 'wav2vec2_prefix_tune_lang_emb_adapter_madg_share'
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        with torch.no_grad() if not ft else contextlib.ExitStack():
            x, padding_mask, cpg_regu_weight = self.w2v_model.extract_features(**w2v_args)
        ...
        return {
            ...
            "padding_mask": padding_mask,
            "cpg_regu_weight": cpg_regu_weight,
        }
