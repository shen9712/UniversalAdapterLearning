...
#         if self.cfg.labels:
#             unit_list = self.cfg.labels.split('.') # 传入的参数如ltr.phn 得到的list为['ltr', 'phn'] # szj 
#             result = {}
#             for unit in unit_list:  
#                 dict_path = os.path.join(
#                     self.cfg.data, f"dict.{unit}.txt"
#                 )
#                 result[unit] = Dictionary.load(dict_path)
#             return result  # 结果是一个词典数据结构，keys为建模单元
...
#             process_label = {}
#             for unit in unit_list:
#                 label_path = os.path.join(data_path, f"{split}.{unit}")
...
#                 assert len(labels[unit]) == len(self.datasets[split]), (
#                         f"labels length ({len(labels)}) and dataset length "
#                         f"({len(self.datasets[split])}) do not match")

#                 process_label[unit] = LabelEncoder(self.target_dictionary[unit])

#             self.datasets[split] = AddTargetDataset(
#                 self.datasets[split],
#                 labels,
#                 pad=self.target_dictionary['ltr'].pad(),
#                 eos=self.target_dictionary['ltr'].eos(),
#                 batch_targets=True,
